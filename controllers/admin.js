...
exports.getDeleteBook = async (req, res, next) => {
    try {
        ...
        req.flash("success", `A book named ${book.bookName} is just deleted!`);
        res.redirect("back");
    } catch (err) {
        console.log(err);
        res.redirect("back");
...
exports.getUserProfile = async (req, res, next) => {
    try {
        ...
        const reviews = await Review.find({ "bookAuthor.id": user_id });
...
exports.postAddNewBook = async (req, res, next) => {
    try {
        ...
        req.flash(
            "success",
            `A new book named ${new_book.bookName} is added to the inventory`
        );
        res.redirect("/admin/bookInventory/all/all/1");
    } catch (err) {
        console.log(err);
