...
exports.postRenewBook = async (req, res, next) => {
    try {
        ...
        const activity = new Activity({
            info: {
                id: issue._id,
                bookName: issue.book_info.bookName,
            },
            category: "Renew",
            time: {
                id: issue._id,
...
/*
    ...
    2. Fetch the book from db and increment its stock by 1
...
exports.postReturnBook = async (req, res, next) => {
    try {
        ...
        // fetching book from db and increment
...
// user -> create new review working procedure
/* 
    1. Find the book to be reviewed by id
    2. Create new Review instance and fill information inside it
    3. Log the activity
...
exports.postNewReview = async (req, res, next) => {
    try {
        ...
        // fetching the book to be reviewed by id
        const book_id = req.params.book_id;
        const book = await Book.findById(book_id);

        // creating new review instance
...
// user -> update existing review working procedure
/*
    1. Fetch the review to be updated from db and update
    2. Fetch the book to be reviewed for logging book id, bookName in activity
    3. Log the activity
    4. Redirect to /books/details/"+book_id
*/
...
// user -> delete existing review working procedure
/* 
    1. Fetch the book info for logging info
    2. Find the position of review id in book.reviews array in Book model
    3. Pop the review id by position from Book
    4. Find the review and remove it from Review
    5. Log the activity
...
exports.deleteReview = async (req, res, next) => {
    ...
    try {
        ...
        // finding the position and popping review_id
        const pos = book.reviews.indexOf(review_id);
        book.reviews.splice(pos, 1);
        await book.save();

        // removing review from Review
        await Reviews.findByIdAndRemove(review_id);
...
exports.deleteUserAccount = async (req, res, next) => {
    try {
        ...
        await Review.deleteMany({ "bookAuthor.id": user_id });
